...
# Глобальный словарь для фиксации времени последней рассылки для каждого пользователя
last_mailing_time = {}
...
async def send_new_words(dp: Dispatcher):
    ...
    for user in users:
        ...
        if user.start_time <= now.strftime("%H:%M") <= user.end_time:
            # Проверяем, прошло ли заданное в настройках пользователя время с последней рассылки
            last_time = last_mailing_time.get(user.chat_id)
            if last_time is None or (now - last_time).total_seconds() >= user.interval_minutes * 60:
                # Обновляем время последней рассылки для этого пользователя
                ...
                if not words:
                    # Если все слова отправлены, сбрасываем прогресс и выбираем слова заново
...
async def send_test_question(dp: Dispatcher):
    ...
    for user in users:
        bot = dp.bot
        chat_id = user.chat_id
        # Выбираем тип теста: 1 – классический или 2 – обратный
...
def start_scheduler(dp: Dispatcher):
    # Запускаем функцию отправки новых слов каждую минуту
